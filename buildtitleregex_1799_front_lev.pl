...
if ($ARGV[0]=~/^wos$/i) {
    ...
    if (!(-e $inputfile)) {
	die("Usage: buildtitleregex_1799_front_lev.pl mag|wos|filename_or_fullpath_of_file\n");
    }
}
...
open(INFILE,$inputfile)||die("Can't open input file >$inputfile<>$sourcefilecode<");
...
$outputdir="$NPL_BASE" . "nplmatch/splittitle_articles/year_regex_scripts_front_" . "$sourcefilecode". "_lev/";

$inputdir="$NPL_BASE" . "nplmatch/splittitle_patent/front/";

$date=`date`;
print "$date";

$curdir="$NPL_BASE" . "nplmatch/splittitle_articles";
chdir("$curdir");
...
#	print "$longestword - $secondlongestword --- $mytitle\n";
