#!/share/pkg.7/perl/5.28.1/install/bin/perl
...
# Indicates if this is a run based on bodytext.  Set using -body command line argument.
my($bodytext)=0;
...
# get Probably Words in a list to filter
my($probablywordspath)="$NPL_MISC" . "probablyonlywords.txt";
print "PROBWORDSPATH: $probablywordspath\n" if ($debug);
print "NPL_MISC: $NPL_MISC\n" if ($debug);
open(PROBABLYWORDS,"$probablywordspath")||die("Can't find probably words file $probablywordspath .\n");
my %ProbablyWord;
while(<PROBABLYWORDS>) {
   my($line)=$_;
   $line=lc($line);
...
# get Dictionary (/usr/share/dict/words) words in a list to filter
my($dictionarypath)="$NPL_MISC" . "dictwords";
open(DICTIONARY,"$dictionarypath")||die("Can't find dictionary file $dictionarypath .\n");
my %EnglishWord;
while(<DICTIONARY>) {
   my($line)=$_;
...
my($commonwordspath)="$NPL_MISC" . "commonwords";
open(INFILE,"$commonwordspath")||die("Can't find commonwords file $commonwordspath .\n");
my(%CommonWords);
while(<INFILE>) {
    my($word)=$_;
    $word=~s/\n//;
...
my($verycommonsurnamespath)="$NPL_MISC" . "verycommonsurnames.csv";
open(INFILE,"$verycommonsurnamespath")||die("Can't open verycommonsurnames file $verycommonsurnamespath .\n");
my(%VeryCommon);
while(<INFILE>) {
    my($line)=$_;
    $line=~s/\s+//;
...
my($somewhatcommonsurnamespath)="$NPL_MISC" . "somewhatcommonsurnames.csv";
open(INFILE,"$somewhatcommonsurnamespath")||die("Can't open somewhatcommonsurnames file $somewhatcommonsurnamespath .\n");
my(%SomewhatCommon);
while(<INFILE>) {
    my($line)=$_;
    $line=~s/\s+//;
...
my($journalabbrevspath)="$NPL_BASE" . "nplmatch/inputs/journalabbrev/journalabbrevs-extended.tsv";
open(JOURNALS,"$journalabbrevspath")||die("Could not open journals abbreviations file $journalabbrevspath .\n");
&ReadJournals();
...
while(<INFILE>) {
    ...
    print "Vol: $vol Issue: $issue FirstPage: $firstpage\n" if ($debug); 
...
#    $lastpageminusone = $lastpage-1; #but you'll have to write two copies of every rule to include this :-(`
    ...
    if ($patentline_numbers) { 
	$patentline_numbers=~s/[^0-9-\.]/ /g; 
	$patentline_numbers=~s/([^0-9\.])\./\1 /g; 
	$patentline_numbers=~s/\.([^0-9\.])/ \1/g; 
    }
	...
	($patentline_numbers=~/[^0-9\.]+$vol\s{1,8}$issue\s{1,8}$firstpage[^0-9\.]+/)&&!
	($year==$vol)&&
    ...
    # vol issue first page with an extra number between vol and issue, high #s
	   ...
	   ($patentline_numbers=~/[^0-9\.]+$vol\s{1,8}\d+\s{1,8}$issue\s{1,8}$firstpage[^0-9\.]+/)&&
	   (!($year==$vol))&&
	   (!($vol<100&&$issue<100&&$firstpage<100))) { 
	$volisspage_score=15; 
    } #
    ...
    # vol issue first page with an extra number between issue and first page, big #s
	   ...
	   ($patentline_numbers=~/[^0-9\.]+$vol\s{1,8}$issue\s{1,8}\d+\s{1,8}$firstpage[^0-9\.]+/)&&
	   (!($year==$vol))&&
	   (!($vol<100&&$issue<100&&$firstpage<100))) { 
	$volisspage_score=15; 
    } 
    ...
    # volume followed by the two pages without other numbers in between should also be very good
	   ...
	   ($patentline_numbers=~/[^0-9\.]+$vol[^0-9\.]{1,8}$firstpage ?- ?$lastpage[^0-9\.]+/)&&
	   (!($year==$vol))&&
    ...
    # also allow for year to come between vol and pages
	   ...
	   ($patentline_numbers=~/[^0-9\.]+$vol[^0-9\.]{1,8}\d{4}[^0-9\.]{1,8}$firstpage ?- ?$lastpage[^0-9\.]+/)&&
	   (!($year==$vol))&&
	   (!($vol<10&&$firstpage<10&&$lastpage<10))) { 
	$volisspage_score=15; 
    } 
    # also the case where lastpage is truncated
	   ...
	   ($patentline_numbers=~/[^0-9\.]+$vol[^0-9\.]{1,8}$firstpage ?- ?\d*(\d)[^0-9\.]+/)&&
	   ($lastpage=~/$1$/)&&
	   (!($year==$vol))&&
	   (!($vol<10&&$firstpage<10&&$lastpage<10))) { 
	$volisspage_score=15; 
    ...
    #volume can come after page page, uncommon
	   ...
	   ($patentline_numbers=~/[^0-9\.]+$firstpage ?- ?$lastpage[^0-9\.]{1,8}$vol[^0-9\.]+/)&&
	   (!($year==$vol))&&
	   (!($vol<10&&$firstpage<10&&$lastpage<10))) { 
	$volisspage_score=15; 
    } 
    elsif ($vol&&$firstpage&&$lastpage&&
	   ($patentline_numbers=~/[^0-9\.]+$firstpage ?- ?\d*(\d)[^0-9\.]{1,8}$vol[^0-9\.]+/)&&
	   ($lastpage=~/$1$/)&&
    ...
    # long volume and long first page alone can be conclusive
	   ...
	   ($patentline_numbers=~/[^0-9\.]$vol[^0-9\.]{1,8}$firstpage[^0-9\.]/)&&
	...
	$volisspage_score=10; 
        ...
        if ($vol && $firstpage) {
            if ($patentline_orig=~/$vol:$firstpage/i) {
              $volisspage_score+=6;
            }
            if ($patentline_orig=~/$vol,\s*$firstpage/i) {
              $volisspage_score+=3;
            }
        }
        if ($issue && $firstpage) {
            if ($patentline_orig=~/$issue:$firstpage/i) {
		$volisspage_score+=6;
            }
	    if ($patentline_orig=~/$issue,\s*$firstpage/i) {
		$volisspage_score+=3;
	    }
        }
	if ($vol && !($vol==$year)) { #MMADD if the volume is the same as the year then this will always match and is meaningless
	    ...
	    elsif ($patentline_numbers=~/[^0-9\.]+$vol[^0-9\.]+/) { 
		$volisspage_score+=2;
	...
	if (($volisspage_score>0)&&$issue&&($patentline_numbers=~/[^0-9\.]+$issue[^0-9\.]+/)) { 
	    $volisspage_score+=2; 
	    # Downgrade single digit numbers as they are much more
	    # likely to happen by coincidence.
...
print "vip=$volisspage_score\n" if ($debug);
		    if ($firstpage>999) { $volisspage_score+=5; } #MMADD if it is four-digit numbers, give a lot of credit
		    elsif ($firstpage>99) { $volisspage_score+=4; }
		    elsif ($firstpage>9) { $volisspage_score+=2; }
		    if ($firstpage==1) {
			if ($patentline_numbers=~/[^0-9\.]+$vol[^0-9\.]+/) {
			    $volisspage_score-=1;
			}
			else {
			    $volisspage_score-=3; 
...
print "pp. partial match\n" if ($debug);
	    ...
	    elsif ($patentline_numbers=~/[^0-9\.]+$firstpage-$lastpage[^0-9\.]+/) { 
		$volisspage_score+=5; 
		if ($firstpage>999) { $volisspage_score+=5; }
		elsif ($firstpage>99) { $volisspage_score+=3; }
		elsif ($firstpage>9) { $volisspage_score+=2; }
		if ($firstpage==1) {
		    if ($patentline_numbers=~/[^0-9\.]+$vol[^0-9\.]+/) {
			$volisspage_score-=1;
	    ...
	    elsif (($patentline_numbers=~/[^0-9\.]+$firstpage-(\d+)[^0-9\.]+/)&& 
		   ($lastpage=~/$1$/)&&
		   (length($firstpage)>length($lastpage))) { 
		$volisspage_score+=4; 
		if ($firstpage>999) { $volisspage_score+=5; }
		...
		if ($patentline_numbers=~/[^0-9\.]+$firstpage[^0-9\.]+/) { 
		    $volisspage_score+=2;
		    # Downgrade single digit numbers as they are much more
		    # likely to happen by coincidence.
		...
		if (($firstpage!=$lastpage)&&($patentline_numbers=~/[^0-9\.]+$lastpage[^0-9\.]+/)) { 
		    $volisspage_score+=2; 
		    # Downgrade single digit numbers as they are much more
		    # likely to happen by coincidence.
	    ...
	    elsif ($patentline_numbers=~/[^0-9\.]+$firstpage-\d+/) { 
		$volisspage_score+=4;
		if ($firstpage==1) { #MMADD
		    $volisspage_score-=2;
		} 
	    }
	    elsif ($patentline_numbers=~/[^0-9\.]+$firstpage-[^0-9\.]+/) { 
		$volisspage_score+=2;  
		if ($firstpage==1) { ##MADD
		    $volisspage_score-=2;
		}
	    ...
	    if (($patentline_numbers=~/[^0-9\.]+$firstpage[^0-9\.]+/)||
		($patentline_numbers=~/[^0-9\.]+$firstpage$/)) {
		if ($firstpage>999) {
		    $volisspage_score+=3;
		}
		if ($firstpage>99) {
...
#	print "Word: $word Common: $common[$wordct] Diff: $difference[$wordct]\n";
    ...
    my($difference);
    foreach $difference (sort(keys %numdifferences)) {
#	print "Diff: $difference Number: $numdifferences{$difference}\n";
	...
	# Figure out position we are matching on
	my($pos)=$mostcommondifference+$wordct-1;
	if ($index[$wordct]) {
		...
		((($difference[$wordct]<=$mostcommondifference+1)&&
		  ($difference[$wordct]>=$mostcommondifference-1))||
		 (("$word" eq "$patent_elements[$pos-1]")||
		  ("$word" eq "$patent_elements[$pos]")||
		  ("$word" eq "$patent_elements[$pos+1]")))) { 
		$title_score+=4; 
	    }
		...
		my($distance)=xs_edistance($word,$patent_elements[$mostcommondifference+$wordct-1]);
		if ($distance==1) { 
		    $title_score+=3; 
		}
#		print "Distance from $word to $patent_elements[$mostcommondifference+$wordct-1] is $distance - MCD: $mostcommondifference Wordct: $wordct\n";
	...
	for($i=1;$i<$numparts;$i+=1) {
	    ...
	    $levdist=xs_edistance($title_orig_nospaces,$maybetitle);
	...
	$confidence+=4;
	print "***Boosting confidence by 4 to $confidence\n" if ($debug);
    }
	...
	print "***Adjusting confidence based on journal score (+ $journal_score ) to $confidence\n" if ($debug);
    }
    ...
    if ($VeryCommon{$surname}||(length($surname)<=2)||$ProbablyWord{$surname}) { ##MM moved the <2 test here instead 
    ...
    if (!(($patentline=~/^$surname\W+/)||($patentline=~/\W+$surname\W+/))) {
        print "INEXACT AUTHOR MATCH\n" if ($debug);
	$inexactauthor = 1;
	# Exact Author Name is Not Present. If author name is 5+ letters long, figure out
	# word that was close to author name and see if it is an English word.  If so,
	# penalize significantly (-5 instead of the -1 or -2 (body) for just being off).
	my($surnamelength)=length($surname);
	if ($surnamelength>=5) {
	    my(@words);
	    @words=split(/\s+/,$patentline);
	    my($goodmatch)=0;
	    my($engwordmatch)="";
	    my($word)="";
	    foreach $word (@words) {
		my($wordlength)=length($word);
		if (($wordlength>($surnamelength+1))||($wordlength<($surnamelength-1))) { next; }
		my($distance)=xs_edistance($surname,$word); 
		if ($distance<=1) {
		    if (!$EnglishWord{$word}) { $goodmatch=1; }
		    else {
			$engwordmatch=$word;
		    }
		}
	    ...
	    if (!$goodmatch) {
		print "No exact author name match.  Best match is English word '$engwordmatch'! Penalizing -5 \n" if ($debug);
		$confidence-=5;
	    ...
	    $confidence-=1;
	    $confidence-=1 if ($bodytext); # extra decrement if body text
	} 
    }
    #print "INEXACTAUTHOR = $inexactauthor\n" if ($debug);
    ...
    if (($surname=~/trademark/)||($surname=~/patent/)||($surname eq "sales")||($surname eq "gate")||($surname eq "czech")||($surname=~/^article/)||($surname eq "rule")||($surname eq "app")||($surname eq "francisco")||($surname eq "berlin")||($surname=~/consultant/||$surname eq "daily")||($surname eq "apple")||($surname eq "daily")||($surname eq "canada")||($surname eq "camp")||($surname eq "chem")||($surname eq "dept")||($surname eq "screening")||($surname=~/^abstract/)||($surname eq "pages")||($surname eq "transaction")||($surname eq "activity")||($surname eq "introduction")||($surname eq "division")||($surname eq "san")||($surname eq "index")||($surname eq "list")||($surname eq "ab")||($surname eq "san")||($surname eq "pages")||($surname eq "ad")||($surname eq "ocean")||($surname eq "other")||($surname eq "we")||($surname eq "to")||($surname eq "pi")||($surname eq "ok")||($surname eq "my")||($surname eq "it")||($surname eq "is")||($surname eq "if")||($surname eq "en")||($surname eq "em")||($surname eq "al")||($surname eq "et")||($surname eq "do")||($surname eq "be")||($surname eq "at")||($surname eq "and")||($surname eq "by")||($surname=~/ieee/)||($surname=~/general/)||($surname=~/paper/)||($surname=~/english/)||($surname eq "usa")||($surname=~/nasa/)||($surname eq "eng")||($surname eq "no")||($surname=~/magazine/)||($surname=~/journal/)||($surname eq "new")||($surname eq "head")||($surname eq "risk")||($surname=~/galaxy/)||($surname eq "show")||($surname eq "org")||($surname eq "array")||(($surname eq "cross")&&($patentline_orig=~/cross\-\w/))) {
        print "useless author <$surname>-> Conf -6\n" if ($debug);
	$confidence-=4; 
    }
    # probably not author names
    if (($surname eq "toy")||($surname eq "plant")||($surname eq "smart")||($surname eq "best")||($surname eq "field")||($surname eq "grant")||($surname eq "gross")||($surname eq "james")||($surname eq "parent")||($surname eq "cross")||($surname eq "back")||($surname eq "north")||($surname eq "rock")||($surname eq "high")||($surname eq "doi")||($surname eq "page")||($surname eq "blood")||($surname eq "baltimore")||($surname eq "san")||($surname eq "ran")||($surname eq "diamond")||($surname eq "america")||($surname eq "toyota")||($surname eq "brain")||($surname eq "power")||($surname eq "borne")||($surname eq "block")||($surname eq "can")) {
	$confidence-=2; 
    }
...
#    if ($patentline_orig=~/^(.{20})/) {
	...
	if (!($probcontains1stauth=~/$surname/) && ($bodytext==0)) {
	...
	$confidence-=2;
        ...
        $inexactyear = 1;
        print "year is off by one! penalizing\n" if ($debug);
	$confidence-=1;
        if ($bodytext==1) {
         print "penalizing even more for wrong year in body\n" if ($debug);
	 $confidence-=2;
        }
    }
    # downweight probable patent filings
    if (($patentline_orig=~/filing date.{1,12}$year/)||($patentline_orig=~/issued as u.?s.? pat.{1,15} on.{1,12}$year/)||($patentline_orig=~/filed .{1,35}\w{1,3}\.? \d{1,2}.{1,5}$year/)||($patentline_orig=~/filed .{1,35}\d{1,2} \w{1,3}.{1,5}$year/)) {
        print "***********************this is a probable patent filing\n" if ($debug);
        $confidence-=5;
    }
    ...
    # if you have inexact name AND inexact year it's probably wrong
    if ($inexactauthor==1 & $inexactyear==1) {
      print "TRUE THAT AUTHOR AND YEAR ARE BOTH INEXACT\n" if ($debug);
        $confidence-=2;
        $confidence-=2 if ($bodytext==1);
    } else {
      print "NOT TRUE THAT AUTHOR AND YEAR ARE BOTH INEXACT: $inexactauthor $inexactyear\n" if ($debug);
    }
