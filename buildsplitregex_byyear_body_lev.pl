...
if ($ARGV[0]=~/^wos$/i) {
    ...
    if (!$inputyear) {
	die("Usage: buildtitleregex_byyear_body_lev.pl mag YEAR|wos YEAR|filename_or_fullpath_of_file\n");
    }
}
elsif ($ARGV[0]=~/^mag$/i) { 
    ...
    if (!$inputyear) {
	die("Usage: buildsplitregex_byyear_body_lev.pl mag YEAR|wos YEAR|filename_or_fullpath_of_file\n");
    }
}
else {
    $inputfile=$ARGV[0];
...
$outputdir="$NPL_BASE" . "nplmatch/splitpagevol_articles/year_regex_scripts_body_" . "$sourcefilecode". "_lev/";;

$inputdir="$NPL_BASE" . "nplmatch/splitpagevol_patent/body/";
...
while (<INFILE>) {
	...
	if ($firstauthor=~/\w/) {
	    if ($matchnumber) {
		if ((length($firstauthor_lastname)>=5)&&(!($firstauthor_lastname=~/ /))) {
...
if ($file==0) {
    ...
    $template_path="$NPL_MISC" . "template_perl_body";
	...
	for($digit=1;$digit<10;$digit++) {
		...
		if ($sizect>=($filesize/8)) { # Because the comparison on the other side (splitpagevol_patents) for single digits is also much larger, treat the file size limit for them as being 1/8 as big as for other numbers.
#		    (($digit==1)||($digit==3))&&($sizect>=($filesize/24))) {
		# Make this even more extreme (1/24) for the single digits 1 and 3 which have much larger files than other numbers.
		    $sizect=0;
		    $filenum=$filect+1000;
